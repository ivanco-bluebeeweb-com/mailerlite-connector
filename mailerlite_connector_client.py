"""HTTP client for MailerLite API."""
from __future__ import annotations
import httpx
from typing import Any, Optional

DEFAULT_BASE = "https://connect.mailerlite.com/api"
...
class MailerLiteClient:
    def __init__(self, api_token: str, base_url: str = ""):
        ...
        self.headers = {
            ...
            "User-Agent": "Imperal-MailerLite-Connector/1.0.0"
        }
    ...
    async def list_subscribers(self, limit: int = 20) -> list[dict[str, Any]]:
        async with httpx.AsyncClient(timeout=self.timeout) as client:
            resp = await client.get(f"{self.base_url}/subscribers", headers=self.headers, params={"limit": limit})
            if resp.status_code == 200:
                data = resp.json()
                if isinstance(data, list): return data
                for k in ["data", "subscribers", "items", "results"]:
